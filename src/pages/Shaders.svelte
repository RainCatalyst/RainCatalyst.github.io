<script>
    import VerticalCard from "../components/ProjectCard.svelte";
</script>

<div class="pt-6 flex flex-col items-start">
    <div class="flex gap-2">
        <a class="highlight-text-big underline" href="https://store.steampowered.com/app/1819520/Constellations_Puzzles_in_the_Sky/">Constellations</a>
        <h1 class="highlight-text-big">shaders</h1>
    </div>
    
    <div class="pt-4 self-stretch grid grid-cols-2 gap-8">
        <VerticalCard name="Procedural Background" link="aag4C4sQ-DQ"/>
        <VerticalCard name="Swirly Blackhole" link="Hg3VU2-0nPU"/>
        <VerticalCard name="World Completion" link="H1MdlJ9X_LI"/>
        <VerticalCard name="Bubble Line" link="5Bmkd89Hrts"/>
    </div>
</div>